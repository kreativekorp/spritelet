...
static int16_t blit(const uint8_t * ptr, int16_t y, uint8_t h) {
  ...
  if ((y < 128) && (y > -h)) {
    ...
    while (n--) {
      m = pgm_read_byte(ptr++);
      i = (m >> 5) & 0x06; SPI_write(pgm_read_byte(&cloud_palette[i])); i++; SPI_write(pgm_read_byte(&cloud_palette[i]));
      i = (m >> 3) & 0x06; SPI_write(pgm_read_byte(&cloud_palette[i])); i++; SPI_write(pgm_read_byte(&cloud_palette[i]));
      i = (m >> 1) & 0x06; SPI_write(pgm_read_byte(&cloud_palette[i])); i++; SPI_write(pgm_read_byte(&cloud_palette[i]));
      i = (m << 1) & 0x06; SPI_write(pgm_read_byte(&cloud_palette[i])); i++; SPI_write(pgm_read_byte(&cloud_palette[i]));
    }
  }
  return r;
